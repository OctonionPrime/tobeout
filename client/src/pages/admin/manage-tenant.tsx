...
const settingsSchema = z.object({
    ...
    status: z.enum(['active', 'suspended', 'terminated', 'trial']),
    timezone: z.string(),
...
const updateTenant = async (tenantId: string, data: Partial<SettingsForm & FeaturesForm>) => {
...
function StatusBadge({ status }: { status: string }) {
    const variants = {
        ...
        trial: { variant: "outline" as const, icon: Clock, color: "text-blue-600", bg: "bg-blue-50" },
...
// ... (Keep TenantMetricsCards, SuspendTenantDialog, ReactivateTenantDialog components as they are)
...
export default function ManageTenantPage() {
    ...
    // Form for tenant features
    const featuresForm = useForm<FeaturesForm>({
        resolver: zodResolver(featuresSchema),
    });

    // Reset forms when tenant data loads
    useEffect(() => {
        if (tenant) {
            ...
            settingsForm.reset({
                ...
                maxReservationsPerMonth: limits.maxReservationsPerMonth,
                adminNotes: tenant.adminNotes,
            });

            featuresForm.reset({
                enableAiChat: features.enableAiChat || false,
                enableTelegramBot: features.enableTelegramBot || false,
                enableGuestAnalytics: features.enableGuestAnalytics || false,
                enableAdvancedReporting: features.enableAdvancedReporting || false,
                enableMenuManagement: features.enableMenuManagement || false,
            });
        }
    }, [tenant, settingsForm, featuresForm]);

    // Update mutation
    const updateMutation = useMutation({
        mutationFn: (data: Partial<SettingsForm & FeaturesForm>) => updateTenant(tenantId, data),
        onSuccess: (data, variables) => {
            toast({
                title: "Tenant Updated",
                description: `Settings for ${Object.keys(variables).join(', ')} saved.`,
            });
    ...
    const onFeaturesSubmit = (data: FeaturesForm) => {
        updateMutation.mutate(data);
    };
                                            ...
                                            <FormField
                                                ...
                                                render={({ field }) => (
                                                    <FormItem>
                                                        ...
                                                        <FormControl><Input {...field} placeholder="America/New_York" /></FormControl>
                                                        <FormMessage />
                                                    </FormItem>
                                                )}
                                            />
                            ...
                            <form onSubmit={featuresForm.handleSubmit(onFeaturesSubmit)}>
                                <Card>
                                    <CardHeader>
                                        ...
                                        <CardDescription>
                                            Enable or disable features for this tenant, overriding plan defaults.
                                        </CardDescription>
                                    </CardHeader>
                                    <CardContent>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <FormField
                                                ...
                                                render={({ field }) => (
                                                    <FormItem className="flex items-center justify-between p-4 border rounded-lg">
                                                        <div>
                                                            <FormLabel className="font-medium">AI Chat Assistant</FormLabel>
                                            ...
                                            <FormField
                                                ...
                                                render={({ field }) => (
                                                    <FormItem className="flex items-center justify-between p-4 border rounded-lg">
                                                        <div>
                                                            <FormLabel className="font-medium">Telegram Bot</FormLabel>
                                            ...
                                            <FormField
                                                control={featuresForm.control}
                                                name="enableGuestAnalytics"
                                                render={({ field }) => (
                                                    <FormItem className="flex items-center justify-between p-4 border rounded-lg">
                                                        <div>
                                                            <FormLabel className="font-medium">Guest Analytics</FormLabel>
                                            ...
                                            <FormField
                                                ...
                                                render={({ field }) => (
                                                    <FormItem className="flex items-center justify-between p-4 border rounded-lg">
                                                        <div>
                                                            <FormLabel className="font-medium">Advanced Reporting</FormLabel>
                                            ...
                                            <FormField
                                                ...
                                                render={({ field }) => (
                                                    <FormItem className="flex items-center justify-between p-4 border rounded-lg">
                                                        <div>
                                                            <FormLabel className="font-medium">Menu Management</FormLabel>
                                    ...
                                    <CardFooter>
                                        {featuresForm.formState.isDirty && (
                                            <Button
                                                type="submit"
                                                disabled={updateMutation.isPending}
